...
Guidance for every agent when working in this repository. The public overview is in [README.md](../README.md);
the machine contract (JSON shapes, exit codes) is in [machine contract](MACHINE_CONTRACT.md). This file captures the
...
Read [ClawHub standards](../.claude/CLAWHUB_STANDARDS.md).
...
Read [CLI conventions](../.claude/CLI_CONVENTIONS.md).
...
The locally bundled CLI conventions are sufficient for work in this checkout. Cross-project convention changes require their existing joint reconciliation process; ordinary tasks do not depend on opening another repository.
